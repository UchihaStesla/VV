'use strict'; let mapping = {};
const dbStorage = {
    dbName: "vvSearchCache", dbVersion: 1, async init() {
        if (this._dbPromise) return this._dbPromise; this._dbPromise = new Promise((resolve, reject) => {
            const request = indexedDB.open(this.dbName, this.dbVersion); request.onerror = event => { reject(event.target.error) }; request.onsuccess = event => { this.db = event.target.result; resolve(this.db) }; request.onupgradeneeded = event => {
                const db = event.target.result; if (!db.objectStoreNames.contains("indices")) db.createObjectStore("indices", { keyPath: "key" }); if (!db.objectStoreNames.contains("mappings")) db.createObjectStore("mappings",
                    { keyPath: "key" }); if (!db.objectStoreNames.contains("databases")) db.createObjectStore("databases", { keyPath: "key" })
            }
        }); return this._dbPromise
    }, async getItem(storeName, key) { try { await this.init(); return new Promise((resolve, reject) => { const transaction = this.db.transaction(storeName, "readonly"); const store = transaction.objectStore(storeName); const request = store.get(key); request.onsuccess = () => { resolve(request.result ? request.result.value : null) }; request.onerror = event => { reject(event.target.error) } }) } catch (error) { return null } },
    async setItem(storeName, key, value) { try { await this.init(); return new Promise((resolve, reject) => { const transaction = this.db.transaction(storeName, "readwrite"); const store = transaction.objectStore(storeName); const request = store.put({ key, value }); request.onsuccess = () => { resolve() }; request.onerror = event => { reject(event.target.error) } }) } catch (error) { throw error; } }, async removeItem(storeName, key) {
        try {
            await this.init(); return new Promise((resolve, reject) => {
                const transaction = this.db.transaction(storeName, "readwrite");
                const store = transaction.objectStore(storeName); const request = store.delete(key); request.onsuccess = () => { resolve() }; request.onerror = event => { reject(event.target.error) }
            })
        } catch (error) { throw error; }
    }, async getAllKeys(storeName) { try { await this.init(); return new Promise((resolve, reject) => { const transaction = this.db.transaction(storeName, "readonly"); const store = transaction.objectStore(storeName); const request = store.getAllKeys(); request.onsuccess = () => { resolve(request.result) }; request.onerror = event => { reject(event.target.error) } }) } catch (error) { return [] } }
};
...
const indexCache = {
        ...
        try {
            ...
            if (header[0] === 0x1f && header[1] === 0x8b) {
                try {
                    ...
                    decompressedData = await new Response(decompressedStream).arrayBuffer();
                } catch (error) {
    ...
    _arrayBufferToBase64(buffer) { const binary = []; const bytes = new Uint8Array(buffer); for (let i = 0; i < bytes.byteLength; i++)binary.push(String.fromCharCode(bytes[i])); return btoa(binary.join("")) },
    
    _base64ToArrayBuffer(base64) {
        const binaryString =
            atob(base64); const len = binaryString.length; const bytes = new Uint8Array(len); for (let i = 0; i < len; i++)bytes[i] = binaryString.charCodeAt(i); return bytes.buffer
    },
    
    _cleanupLocalStorage() { try { const cacheKeys = []; for (let i = 0; i < localStorage.length; i++) { const key = localStorage.key(i); if (key.startsWith("indexCache_")) cacheKeys.push(key) } if (cacheKeys.length > 20) for (let i = 0; i < 5; i++)localStorage.removeItem(cacheKeys[i]) } catch (e) { } }
}; const watermarkImage = new Image; watermarkImage.src = "watermark.png";
let watermarkLoaded = false; watermarkImage.onload = () => { watermarkLoaded = true };
    ...
    return RequestController.enqueue(requestKey, async () => {
        try {
            ...
            if (response.status === 416 || !response.ok) {
                ...
                if (!fullResponse.ok) {
                    throw new Error(`HTTP error: ${fullResponse.status} ${fullResponse.statusText}`);
                }
            ...
            console.error(`Error extracting frame ${frameNum} from folder ${folderId}:`, error);
...
async function loadMapping() {
    try {
        const cachedMapping = await dbStorage.getItem("mappings", "mapping"); if (cachedMapping) return cachedMapping; try { const localMapping = localStorage.getItem("mapping"); if (localMapping) { const mappingData = JSON.parse(localMapping); try { await dbStorage.setItem("mappings", "mapping", mappingData); localStorage.removeItem("mapping") } catch (e) { } return mappingData } } catch (e) { } const response = await fetch("./mapping.json", {
            referrerPolicy: 'no-referrer',
            mode: 'cors',
            credentials: 'omit'
        }); if (!response.ok) throw new Error("Failed to load mapping.json"); const mappingData =
...
class UIController {
    static updateSearchFormPosition(isSearching) { const searchForm = document.getElementById("searchForm"); const randomStringDisplay = document.getElementById("randomStringDisplay"); if (isSearching) { searchForm.classList.add("searching"); if (!AppState.randomStringDisplayed) this.showRandomString() } else { searchForm.classList.remove("searching"); this.clearRandomString() } } static showRandomString() {
        if (!AppState.randomStringDisplayed) {
            ...
            const randomIndex = Math.floor(Math.random() * CONFIG.randomStrings.length); randomStringDisplay.textContent = CONFIG.randomStrings[randomIndex]; AppState.randomStringDisplayed = true; randomStringDisplay.classList.remove("fade-out"); randomStringDisplay.classList.add("fade-in")
        }
    } static clearRandomString() {
        const randomStringDisplay = document.getElementById("randomStringDisplay"); randomStringDisplay.classList.remove("fade-in"); randomStringDisplay.classList.add("fade-out"); setTimeout(() => {
...
async function handleSearch(event) {
    ...
    try {
        ...
        document.getElementById("errorDisplay").textContent = `搜索失败: ${error.message}`;
...
async function initializeApp() {
    try {
        ...
        if (window.subtitleDB && !window.subtitleDB.isLoaded && !window.subtitleDB.isLoading) {
            window.subtitleDB.load().then(() => {
...
document.addEventListener("DOMContentLoaded", () => {
    const loadingBar = document.getElementById("loadingBar"); if (loadingBar) { loadingBar.style.width = "0%"; loadingBar.style.display = "none" } initializeApp(); const toggleButton = document.getElementById("toggleAdvancedOptions"); const advancedOptions = document.getElementById("advancedOptions"); toggleButton.addEventListener("click", () => {
        const isExpanded = advancedOptions.classList.contains("show"); if (!isExpanded) {
            advancedOptions.style.transition = "none"; advancedOptions.classList.add("show");
            const height = advancedOptions.scrollHeight; advancedOptions.classList.remove("show"); void advancedOptions.offsetHeight; advancedOptions.style.transition = ""; advancedOptions.style.maxHeight = height + "px"; advancedOptions.classList.add("show")
        } else { advancedOptions.style.maxHeight = "0"; advancedOptions.classList.remove("show") } toggleButton.classList.toggle("active"); toggleButton.setAttribute("aria-expanded", !isExpanded)
    }); const watermarkToggle = document.getElementById("watermarkToggle"); watermarkToggle.addEventListener("change",
        () => {
            AppState.showWatermark = watermarkToggle.checked; if (window.canvasRenderQueue) window.canvasRenderQueue.forEach(canvas => {
                const ctx = canvas.getContext("2d"); ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.drawImage(canvas.originalCanvas, 0, 0); if (AppState.showWatermark && watermarkLoaded) {
                    const watermarkScale = canvas.width * .25 / watermarkImage.width; const watermarkWidth = watermarkImage.width * watermarkScale; const watermarkHeight = watermarkImage.height * watermarkScale; ctx.drawImage(watermarkImage, canvas.width -
                        watermarkWidth - 5, canvas.height - watermarkHeight - 5, watermarkWidth, watermarkHeight)
                }
            })
        })
});
function displayResults(data, append = false) {
    ...
    if (data.data && data.data.length === 1 && data.data[0].count === 0) {
        ...
        if (!append) {
            const message = noResultData.message || `未找到与 "${document.getElementById("query").value.trim()}" 匹配的结果`;
    ...
    const cards = newResults.map(result => {
        ...
        const cleanFilename = result.filename ? result.filename.replace(/\[P(\d+)\].*?\s+/, "P$1 ").replace(/\.json$/, "").trim() : ""; 
...
} function getEpisodeUrl(filename) { for (let key in mapping) if (mapping[key] === filename) return key; return null }
function startNaturalLoadingBar() {
    const loadingBar = document.getElementById("loadingBar"); loadingBar.style.transition = ""; loadingBar.style.width = "0%"; loadingBar.style.display = "block"; if (loadingBar.interval) clearInterval(loadingBar.interval); let progress = 0; const targetProgress = 95; let speed = .5; loadingBar.interval = setInterval(() => {
        if (progress < 30) speed = .8; else if (progress < 60) speed = .4; else if (progress < 80) speed = .2; else speed = .1; progress += speed; if (progress >= targetProgress) {
            clearInterval(loadingBar.interval);
            progress = targetProgress
        } loadingBar.style.width = `${progress}%`
    }, 50)
} function completeLoadingBar() { const loadingBar = document.getElementById("loadingBar"); clearInterval(loadingBar.interval); loadingBar.style.transition = "width 0.3s ease-out"; loadingBar.style.width = "100%" }
...
function handleCardClick(result) {
    const episodeMatch = result.filename.match(/\[P(\d+)\]/); const timeMatch = result.timestamp.match(/^(\d+)m(\d+)s$/); if (episodeMatch && timeMatch) {
        const episodeNum = parseInt(episodeMatch[1], 10); const minutes = parseInt(timeMatch[1]); const seconds = parseInt(timeMatch[2]); const totalSeconds = minutes * 60 + seconds; for (const [url, filename] of Object.entries(mapping)) if (filename === result.filename) {
            const videoUrl = `https://www.bilibili.com${url}?t=${totalSeconds}`; window.open(videoUrl, "_blank");
            break
        }
    }
}
